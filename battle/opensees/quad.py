"""
OpenSees quad PlaneStrain cantilever
SIMPLEFEM quad4 と同じ 55節点×40要素メッシュ、同じ荷重
bilinear quad4 → せん断ロッキングが生じる
"""
import sys, os, time
...
for r in range(4):
    for c in range(10):
        ...
        n3 = (r + 1) * 11 + c + 2
        n4 = (r + 1) * 11 + c + 1
        ops.element('quad', eid, n1, n2, n3, n4, T, 'PlaneStrain', 1)
        eid += 1
...
print(f"OPENSEES_QUAD_TIP_UY={uy_tip:.8f}")
...
with open(os.path.join(result_dir, 'quad_uy.txt'), 'w') as f:
